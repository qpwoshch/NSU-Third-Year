...
pthread_cond_t queue_cond1;
pthread_cond_t queue_cond2;
...
queue_t* queue_init(int max_count) {
    ...
    err = pthread_cond_init(&queue_cond1, NULL);
    if (err) {
        printf("queue_init: pthread_cond_init() failed: %s\n", strerror(err));
        abort();
    }

    err = pthread_cond_init(&queue_cond2, NULL);
    if (err) {
        printf("queue_init: pthread_cond_init() failed: %s\n", strerror(err));
        abort();
    }
...
void queue_destroy(queue_t *q) {
    ...
    pthread_cond_destroy(&queue_cond1);
...
int queue_add(queue_t *q, int val) {
    ...
    while (q->count == q->max_count) {
        pthread_cond_wait(&queue_cond1, &queue_lock);
    }
...
int queue_get(queue_t *q, int *val) {
    ...
    pthread_mutex_lock(&queue_lock);
    while (q->count == 0) {
        pthread_cond_wait(&queue_cond2, &queue_lock);
    }
    qnode_t *tmp = q->first;
    *val = tmp->val;
    ...
    q->count--;
    q->get_count++;
    pthread_cond_broadcast(&queue_cond1);
    pthread_mutex_unlock(&queue_lock);
    return 1;
}
